...
#define BOOSTPULSE_ONDEMAND "/sys/devices/system/cpu/cpu0/cpufreq/ondemand/boostpulse"
#define BOOSTPULSE_INTERACTIVE "/sys/devices/system/cpu/cpu0/cpufreq/interactive/boostpulse"
#define FREQ_BUF_SIZE 10

static char scaling_max_freq[FREQ_BUF_SIZE]   = "768000";
static char screenoff_max_freq[FREQ_BUF_SIZE] = "245760";
...
static void qsd8k_power_set_interactive(struct power_module *module, int on)
{
    ...
    ALOGI("Set max frequency to " + on ? "768" : "245");
    ...
    if (get_scaling_governor(governor, sizeof(governor)) < 0) {
        ...
        if (strncmp(governor, "ondemand", 8) == 0)
            sysfs_write("/sys/devices/system/cpu/cpu0/cpufreq/ondemand/sampling_rate",
                        on ? "50000" : "500000");
    }

    /* Increase ksm sleep interval */
    ALOGI("Set KSM sleep to" + on ? "1500" : "7500");
