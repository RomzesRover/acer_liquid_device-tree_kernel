 ...
 * Copyright (C) 2013 The cm10 for liquid Project
...
static char scaling_max_freq[FREQ_BUF_SIZE]   = "960000";
static char screenoff_max_freq[FREQ_BUF_SIZE] = "245000";
...
static void sysfs_write(char *path, char *s)
{
    ...
    if (fd < 0) {
        ...
        ALOGE("POWER MODULE, Error opening %s: %s\n", path, buf);
        return;
    }
    ...
    if (len < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("POWER MODULE, Error writing to %s: %s\n", path, buf);
    }
...
static int sysfs_read(char *path, char *s, size_t l)
{
    ...
    if (len < 0) {
        strerror_r(errno, buf, sizeof(buf));
        ALOGE("POWER MODULE, Error reading from %s: %s\n", path, buf);
    } else {
...
static int boostpulse_open(struct qsd8k_power_module *qsd8k)
{
    ...
    if (qsd8k->boostpulse_fd < 0) {
        if (get_scaling_governor(governor, sizeof(governor)) < 0) {
            ALOGE("POWER MODULE, Can't read scaling governor.");
            ...
            if (qsd8k->boostpulse_fd < 0 && !qsd8k->boostpulse_warned) {
                strerror_r(errno, buf, sizeof(buf));
                ALOGE("POWER MODULE, Error opening %s boostpulse interface: %s\n", governor, buf);
                qsd8k->boostpulse_warned = 1;
                            ...
                            void *data)
{
    ...
    case POWER_HINT_CPU_BOOST:
        if (boostpulse_open(qsd8k) >= 0) {
            ...
            if (len < 0) {
                strerror_r(errno, buf, sizeof(buf));
                ALOGE("POWER MODULE, Error writing to boostpulse: %s\n", buf);
                pthread_mutex_lock(&qsd8k->lock);
