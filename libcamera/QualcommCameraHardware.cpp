...
#define LOG_TAG "AcerCameraHardware"
#include <utils/Log.h>
...
#define CAMERA_IOCTL_TIMEOUT_MS  5000
...
/* The support requires an algorithm. This was implemented in the qcom proprietary libraries */
#define ENABLE_ISO_SUPPORT 1
...
static int fb_fd = -1;
static uint8_t mMaxZoom = -1;

static int dstOffset = 0;

static int cameracontrolfd;
pthread_t w_control_thread;

#if 0
static int cameraconfigfd;
pthread_t w_config_thread;
#endif

void *opencameracontrolfd(void *data) {
    cameracontrolfd = open(MSM_CAMERA_CONTROL, O_RDWR);
    return NULL;
}

#if 0
void *opencameraconfigfd(void *data) {
    cameraconfigfd = open(MSM_CAMERA_CONFIG, O_RDWR);
    return NULL;
}
#endif
      ...
      mDebugFps(0)
{
    ...
    // needed.
    if ((pthread_create(&w_control_thread, NULL, opencameracontrolfd, NULL)) != 0) {
...
void QualcommCameraHardware::initDefaultParameters()
{
    ...
    mParameters.set(CameraParameters::KEY_ZOOM_RATIOS, "100,200,300,400,500,600");
    mParameters.set(CameraParameters::KEY_MAX_ZOOM, (int)MAX_ZOOM_LEVEL);
    mParameters.set(CameraParameters::KEY_ZOOM, (int)CAMERA_DEF_ZOOM);
    ALOGE("%s: default zoom is set to: %f", __FUNCTION__, (float)mParameters.getInt(CameraParameters::KEY_ZOOM));
...
*/
    ...
    if (pthread_join(w_control_thread, NULL) != 0) {
        ALOGE("Camera open thread exit failed");
        return false;
    }
    mCameraControlFd = cameracontrolfd;
    ...
    if ((pthread_create(&mCamConfigThread, NULL, LINK_cam_conf, NULL)) != 0) {
        ALOGE("startCamera: linking cam_conf failed");
    }

    if (pthread_join(mCamConfigThread, NULL) != 0) {
        ALOGE("Camera config thread exit failed");
        return false;
    }

    if ((pthread_create(&w_config_thread, NULL, opencameraconfigfd, NULL)) != 0) {
        ALOGE("Camera open thread creation failed");
    }

    mCameraConfigFd = cameraconfigfd;

    if (mCameraConfigFd < 0) {
        ALOGE("startCamera X: %s open failed: %s!",
             MSM_CAMERA_CONFIG,
             strerror(errno));
        return false;
                                      ...
                                      const Vector<String16>& args) const
{
    ...
    snprintf(buffer, 255, "mMsgEnabled (0x%x)\n", mMsgEnabled);
...
static bool native_prepare_snapshot(int camfd)
{
    ...
    ALOGV("native_prepare_snapshot: E\n");
    ...
    ALOGV("native_prepare_snapshot: X\n");
    return true;
}
...
static bool native_start_raw_snapshot(int camfd)
{
    int ret;
    struct msm_ctrl_cmd ctrlCmd;
    ALOGV("native_start_raw_snapshot: E\n");
    ...
    ALOGV("native_start_raw_snapshot: X\n");
    return true;
}
...
static bool native_start_recording(int camfd)
{
    int ret;
    struct msm_ctrl_cmd ctrlCmd;
    ALOGV("native_start_recording: E\n");
   ...
   *       PP status should be in  ctrlCmd */
    ...
    ALOGV("native_start_recording: X\n");
    return true;
}
...
bool QualcommCameraHardware::native_set_cfg(
    cam_ctrl_type type, int8_t data)
{
    struct sensor_cfg_data scfg;

    scfg.cfgtype = type;
    scfg.cfg.focus.dir = 1;
    scfg.cfg.focus.steps = data;

    ALOGD("%s: type %d, value %d\n", __FUNCTION__, type, data);
    int status = ioctl(mCameraControlFd, MSM_CAM_IOCTL_SENSOR_IO_CFG, &scfg);
    if( status < 0 ) {
        ALOGE("native_set_cfg: ioctl failed");
        return false;
    }
...
initJpegHeap = true;
    ...
    ALOGV("initRaw E: picture size=%dx%d, initJpegHeap:%d", rawWidth, rawHeight, initJpegHeap);
...
void QualcommCameraHardware::runAutoFocus()
{
    ...
    if ((mParameters.get(CameraParameters::KEY_FOCUS_MODE) == 0)
        ...
        ALOGV("AF: focus is disabled or at infinity. Not doing anything");
        goto done;
    }
...
{
    ALOGV("takePicture(0x%x)", mMsgEnabled);
    Mutex::Autolock l(&mLock);
    ...
    if(mSnapshotFormat == PICTURE_FORMAT_JPEG){
        ALOGE("calling initRaw 0x%x, 0x%x, 0x%x. bit:0x%x, jpegFlag:%d", mDataCallback, mMsgEnabled, 
					CAMERA_MSG_COMPRESSED_IMAGE, (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE), (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)));
...
// If disabled preview is lost after restart
    ALOGV("setParameters: setting orientation");
    ...
    if(final_rc)
		ALOGE("%s:%s:%d\n", __FILE__, __FUNCTION__, __LINE__);
    ALOGV("setParameters: setting lens shade value");
    ...
    if(final_rc)
		ALOGE("%s:%s:%d\n", __FILE__, __FUNCTION__, __LINE__);
#endif
    ALOGV("setParameters: setting picture format");
    ...
    if(final_rc)
		ALOGE("%s:%s:%d\n", __FILE__, __FUNCTION__, __LINE__);
    ALOGV("setParameters: setting contrast");
        ...
        if (new_focus_area != NULL) {
            ...
            if (!area.isDummy()) {
                int width, height, frame_size;
                 /*if (obj != 0) {
                    obj->getPreviewSize(&width, &height, &frame_size);
                 }*/
                ...
                /*if ((obj ==0) || (obj->setObjectPosition(x, y) < 0)) {
                    ALOGE("ERR(%s):Fail on obj->setObjectPosition(%s)", __func__, new_focus_area);
...
sp<CameraHardwareInterface> QualcommCameraHardware::createInstance()
{
    ...
    ALOGD("createInstance: X created camera=%p", cam);
        ...
        if( ( mCurrentTarget == TARGET_MSM7630 ) || (mCurrentTarget == TARGET_QSD8250))  {
            ...
            for(int cnt = 0; cnt < kRecordBufferCount; cnt++) {
                if(record_buffers_tracking_flag[cnt] == true) {
                    ALOGI("Dangling buffer: offset = %d, buffer = %d", cnt, recordframes[cnt].buffer);
...
void QualcommCameraHardware::notifyShutter(common_crop_t *crop, bool mPlayShutterSoundOnly)
{
    ...
    ALOGV("notifyShutter: E %d", mPlayShutterSoundOnly);
    ...
    if(mPlayShutterSoundOnly) {
        ...
        mNotifyCallback(CAMERA_MSG_SHUTTER, 0, mPlayShutterSoundOnly,
                            mCallbackCookie);
        // Hack: Re-enable msg shutter just in case the mmcamera disables it.
    ...
    if (mShutterPending && mNotifyCallback && (mMsgEnabled & CAMERA_MSG_SHUTTER)) {
        ALOGV("out2_w=%d, out2_h=%d, in2_w=%d, in2_h=%d",
             mCrop.out2_w, mCrop.out2_h, mCrop.in2_w, mCrop.in2_h);
        ALOGV("out1_w=%d, out1_h=%d, in1_w=%d, in1_h=%d",
             mCrop.out1_w, mCrop.out1_h, mCrop.in1_w, mCrop.in1_h);
...
void QualcommCameraHardware::receiveRawPicture()
{
    ...
    if (mDataCallback && (mMsgEnabled & CAMERA_MSG_RAW_IMAGE)) {
        ...
        if (mCrop.in2_w != 0 && mCrop.in2_h != 0 &&
            ...
            // By the time native_get_picture returns, picture is taken. Call
            // shutter callback if cam config thread has not done that.
            ALOGV("notifying shutter after copying");
            notifyShutter(&mCrop, FALSE);
        }
    ...
    if (mDataCallback && (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE)) {
        ...
        else ALOGE("receiveRawPicture X: jpeg_encoder_init failed.");
    }
...
#if 0
               // hack: Hardcode auto to ISO 200 for now.
               if (temp == CAMERA_ISO_AUTO) {
                  temp = CAMERA_ISO_200;
                  mParameters.set(CameraParameters::KEY_ISO_MODE, "ISO200");
                  ALOGE("Setting ISO mode to: ISO200");
                  return NO_ERROR;
               }
#endif
            }
...
int QualcommCameraHardware::sensor_io_cfg(int s_cfg_type, int s_cfg_mode, int s_cfg_rs, uint8_t s_cfg_max_steps, void *s_cfg_data_p)
{
    struct sensor_cfg_data s_cfg_data_buf;
    int rc = 0;

    s_cfg_data_buf.cfgtype=s_cfg_type;
    s_cfg_data_buf.mode=s_cfg_mode;
    s_cfg_data_buf.rs=s_cfg_rs;
    s_cfg_data_buf.max_steps=s_cfg_max_steps;

    /* Initialize the correct variable that will be filled up */
    switch(s_cfg_type)
    {
        case CFG_GET_PREV_P_PL:
            s_cfg_data_buf.cfg.prevp_pl=0;
        break;
        case CFG_GET_AF_MAX_STEPS:
            s_cfg_data_buf.max_steps=0;
        break;
        case CFG_MOVE_FOCUS:
            s_cfg_data_buf.cfg.focus.steps=(*((struct focus_cfg *)s_cfg_data_p)).steps;
            s_cfg_data_buf.cfg.focus.dir=(*((struct focus_cfg *)s_cfg_data_p)).dir;
            break;
    ...
    /* Do the ioctl system call and save the return code in rc */
    if((rc=ioctl(mCameraControlFd, MSM_CAM_IOCTL_SENSOR_IO_CFG, (void *)&s_cfg_data_buf )) < 0) {
        ALOGE("%s: ioctl failed error %s\n", __FUNCTION__, strerror(errno));
    }

    /* Then copy the correct variable in s_cfg_data_p */
    switch(s_cfg_type)
    {
        case CFG_GET_PREV_P_PL:
        {
            uint16_t *s_cfg_data_buf_prevp_pl=
                (uint16_t *) s_cfg_data_p;
            *s_cfg_data_buf_prevp_pl=
                s_cfg_data_buf.cfg.prevp_pl;
        }
        break;
        case CFG_GET_AF_MAX_STEPS:
        {
            uint8_t *s_cfg_data_buf_max_steps=
                (uint8_t *) s_cfg_data_p;
            *s_cfg_data_buf_max_steps=
                s_cfg_data_buf.max_steps;
        }
        break;
    }
...
{
    unsigned char rc = NO_ERROR;
    focus_cfg *pZoom = (focus_cfg *)pZm;
    int ioctlRetVal;
    ...
    ctrlCmd.timeout_ms = CAMERA_IOCTL_TIMEOUT_MS;
    ctrlCmd.length         = sizeof(int32_t);
    ctrlCmd.value          = (void *)&pZoom->steps;

    if((ioctlRetVal = ioctl(camfd, MSM_CAM_IOCTL_CTRL_COMMAND_2, &ctrlCmd)) < 0) {
        rc = BAD_VALUE;
    }
    ALOGV("native_set_zoom: control command status is %d error string is: %s\n", ioctlRetVal, strerror(errno));

    memcpy(&pZoom->steps, (int32_t *)ctrlCmd.value,sizeof(int32_t));
...
{
    status_t rc = NO_ERROR;
    struct focus_cfg focusCfg;

    mZoomValueCurr = (float)params.getInt(CameraParameters::KEY_ZOOM);
    ALOGV("%s: mZoomValueCurr:%f,  mZoomValuePrev:%f", __FUNCTION__, mZoomValueCurr, mZoomValuePrev);

    if (mZoomValuePrev != mZoomValueCurr) {
        unsigned int ZoomDirection = MOVE_NEAR;
        if (mZoomValuePrev > mZoomValueCurr) {
            ZoomDirection = MOVE_FAR;
        } else {
            ZoomDirection = MOVE_NEAR;
        }

        focusCfg.dir = ZoomDirection;
        focusCfg.steps = params.getInt(CameraParameters::KEY_ZOOM);
        ALOGV("%s: New ZOOM value : %d", __FUNCTION__, focusCfg.steps);
        rc = native_set_zoom(mCameraControlFd, (void *)&focusCfg);
...
void QualcommCameraHardware::enableMsgType(int32_t msgType)
{
    Mutex::Autolock lock(mLock);
    mMsgEnabled |= msgType;
    ALOGV("enableMsgType: after 0x%x",  mMsgEnabled);
}
...
void QualcommCameraHardware::disableMsgType(int32_t msgType)
{
    Mutex::Autolock lock(mLock);
    mMsgEnabled &= ~msgType;
    ALOGV("disableMsgType: after 0x%x",  mMsgEnabled);
}
...
#if 0
int QualcommCameraHardware::getPreviewSize(int *width, int *height, int *frame_size) {
    *width = previewWidth;
    *height = previewHeight;
    *frame_size = mPreviewFrameSize;

    return 0;
}

int QualcommCameraHardware::setObjectPosition(int x, int y) {
